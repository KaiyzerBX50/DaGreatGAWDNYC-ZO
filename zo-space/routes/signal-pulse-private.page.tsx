...
export default function SignalPulsePrivatePage() {
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem("signal_pulse_passcode") || "";
      if (v) setPasscode(v);
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    try {
      if (passcode) localStorage.setItem("signal_pulse_passcode", passcode);
      else localStorage.removeItem("signal_pulse_passcode");
    } catch {
      // ignore
    }
  }, [passcode]);
  ...
  async function run() {
    ...
    try {
      const r = await fetch("/api/signal-pulse-private", {
        method: "POST",
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-10">
        <div className="flex flex-col items-center gap-6 text-center mb-12">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-bold bg-gradient-to-r from-cyan-400 via-blue-400 to-blue-500 bg-clip-text text-transparent pulse-title">
              Zo Signal Pulse
            </h1>
            <p className="mt-3 text-lg text-zinc-300/80 max-w-2xl mx-auto">Turn meeting notes into clear ownership</p>
            <p className="mt-2 text-sm text-zinc-400">This page requires your passcode. It uses your Zo credits. Reports save into your workspace.</p>
          </div>
          ...
          <GlassCard className="p-5">
            ...
            <div className="mt-5 grid gap-4">
              <div className="flex justify-center">
                <div className="grid gap-2 w-[520px] max-w-full">
                  <label className="text-xs font-semibold uppercase text-zinc-400 text-center">Passcode</label>
                  <div className="flex items-center gap-2">
                    <input
                      value={passcode}
                      onChange={(e) => setPasscode(e.target.value)}
                      type={showPasscode ? "text" : "password"}
                      className="w-full h-11 rounded-xl bg-zinc-950/35 px-3 py-2 text-sm outline-none ring-1 ring-white/10 focus:ring-emerald-400/40"
                    ...
                    <button
                      ...
                      className={
                        ...
                        (showPasscode ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-200" : "border-white/10 bg-zinc-950/35 text-zinc-200 hover:border-white/20 hover:bg-zinc-950/55")
                      }
                    >
                  ...
                  <div className="text-[11px] text-zinc-500">Saved in your browser storage on this device.</div>
                </div>
              </div>
              ...
              <div className="text-[11px] text-zinc-500">This route requires your passcode. Do not share your passcode in notes.</div>
            </div>
          </GlassCard>
          ...
          <div className="grid gap-6">
            <GlassCard className="p-5">
              ...
              <div className="mt-5 grid gap-4">
                ...
                {health && (!health.has_zo_api_key || !health.has_passcode) ? (
                  <div className="rounded-xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-xs text-amber-100">
                    <div className="font-medium text-amber-200">Setup required</div>
                    <ol className="mt-2 list-decimal pl-4 grid gap-1 text-amber-100/90">
                      {!health.has_zo_api_key ? (
                        <li>
                          Create your own Zo access token. Save it as secret <code className="text-amber-50">ZO_API_KEY</code>.
                        </li>
                      ) : null}
                      {!health.has_passcode ? (
                        <li>
                          Set secret <code className="text-amber-50">SIGNAL_PULSE_PASSCODE</code>. Pick a long passcode.
                        </li>
                      ) : null}
                    </ol>
                    <div className="mt-2 text-[11px] text-amber-100/70">This private runner runs AI and uses your credits. Never use someone else’s key.</div>
                  </div>
                ) : null}
              ...
              <GlassCard className="p-5">
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-sm text-zinc-200">Protected</div>
                      <div className="text-xs text-zinc-500">Passcode is required on every run.</div>
                    </div>
                  </div>
                </div>
              </GlassCard>
