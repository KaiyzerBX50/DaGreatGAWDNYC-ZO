...
// In-memory abuse protection. Resets on deploy.
const ipBuckets = new Map<string, { windowStart: number; count: number; lastAt: number }>();
const WINDOW_MS = 60 * 60 * 1000;
const MAX_REQ_PER_WINDOW = 1000;
const MIN_GAP_MS = 0;
...
function rateLimitOk(ip: string) {
  ...
  if (!cur) {
    ipBuckets.set(ip, { windowStart: now, count: 1, lastAt: now });
  ...
  if (MIN_GAP_MS > 0 && now - cur.lastAt < MIN_GAP_MS) {
    return { ok: false as const, reason: "cooldown" };
  }

  if (cur.count >= MAX_REQ_PER_WINDOW) {
    return { ok: false as const, reason: "window" };
  }
...
export default async (c: Context) => {
  try {
    ...
    if (!rl.ok) {
      return c.json(
        {
          error: rl.reason === "cooldown" ? "Slow down" : "Too many requests",
          status: 429,
        },
        429
      );
    ...
    if (notes.length > MAX_NOTES_CHARS) {
      return c.json({ error: `Notes too long. Max ${MAX_NOTES_CHARS} characters.` }, 413);
    }
    ...
    if (!requiredPasscode) {
      return c.json(
        {
          error: "Missing SIGNAL_PULSE_PASSCODE",
          setup: "Set secret SIGNAL_PULSE_PASSCODE (Settings > Advanced > Secrets) to protect your private Signal Pulse route.",
        },
        500
      );
    }

    if (!constantTimeEqual(passcode, requiredPasscode)) {
      ...
      return c.json(
        {
          ...
          setup: "Create a Zo access token in Settings > Advanced > Access Tokens, then save it as secret ZO_API_KEY.",
        },
        500
      );
    }
